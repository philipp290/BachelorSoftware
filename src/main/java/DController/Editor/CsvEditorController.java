...
public class CsvEditorController {
    private final CsvEditorWindow view;
    ...
    public CsvEditorController(CsvEditorWindow view) {
        this.view = view;
        wire();
    }
        ...
        view.addDeleteListener((ActionEvent e) -> {
            if(!displayedFile.isEmpty()) {
                if (!view.getDeleteCol().isEmpty() && (Integer.parseInt(view.getDeleteCol()) > 0) && (Integer.parseInt(view.getDeleteCol()) < colCount)) {
                    ...
                    view.showError("Es wurde eine invalide / keine Spalte zum löschen ausgewählt");
                }
            }else{
    ...
    private void loadCSVToTable(String filePath) {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(filePath))) {
            ...
            view.showError("Fehler beim Datei-Lesen");
        }
    }
}
